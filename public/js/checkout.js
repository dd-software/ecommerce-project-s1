        ...
        paypal.Buttons({
            ...
            onApprove: async (data, actions) => {
                try {
                    ...
                    if (captureData.success) {
                        ...
                        // Redirigir a mis-pedidos con banner de éxito
                        setTimeout(() => {
                            const pedidoId = captureData.data?.pedido_id || '';
                            window.location.href = App.getBasePath() + '/mis-pedidos.html?pago=exitoso&pedido_id=' + pedidoId;
                        }, 1500);
                    } else {
            ...
            onCancel: async (data) => {
                ...
                try {
                    ...
                    console.warn('[PayPal] No se pudo reactivar carrito (puede que ya esté activo):', e.message);
                }
                if (btn) btn.classList.remove('d-none');
                paypalContainer.classList.add('d-none');
                // Recargar datos del carrito
