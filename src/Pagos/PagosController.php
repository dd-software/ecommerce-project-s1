...
class PagosController
{
    ...
    {
        try {
            // 1. Validar firma / secreto compartido del Webhook
            // Nota: En producción, se suele verificar una firma HMAC con una clave secreta.
            $secretHeader = $request->getHeader('X-Webhook-Secret') ?? '';
            $webhookSecretConfig = 'whsec_live_pixelstorage_2026'; // Clave secreta simulada

            if (empty($secretHeader) || $secretHeader !== $webhookSecretConfig) {
                $response->error('UNAUTHORIZED_WEBHOOK', 'Firma o secreto de webhook inválido.', 401);
                return;
            }
            ...
            // 2. Procesar en el servicio (el servicio controlará que no esté duplicado)
            $this->service->procesarWebhook($pedidoId, $estado, $transaccionId);

            $response->json(['mensaje' => 'Webhook procesado exitosamente.']);

        } catch (\RuntimeException $e) {
            // Capturar errores de lógica de negocio (ej: si ya estaba pagado pero queremos avisar)
            $response->json(['mensaje' => $e->getMessage()]);
        } catch (\Exception $e) {
